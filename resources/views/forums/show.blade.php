            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-body p-4 p-md-5">
                    <div class="d-flex align-items-center mb-4">
                        <img src="{{ $forum->user->foto_profil ?? 'https://ui-avatars.com/api/?name='.urlencode($forum->user->name) }}" class="rounded-circle me-3" width="50" height="50">
                        <div>
                    ...
                    <h2 class="fw-bold mb-4">{{ $forum->judul_diskusi }}</h2>
                    <div class="text-dark opacity-90 mb-5 fs-5 leading-relaxed" style="white-space: pre-line;">
                        {{ $forum->deskripsi_masalah }}
                    </div>
                    <hr class="opacity-10 mb-5">

                    <!-- Comments Section -->
                    <h5 class="fw-bold mb-4">Diskusi ({{ $forum->comments->count() }})</h5>
                    
                    <div class="comments-list mb-5">
                        @foreach($forum->comments as $comment)
                            <div class="d-flex mb-4">
                                <img src="{{ $comment->user->foto_profil ?? 'https://ui-avatars.com/api/?name='.urlencode($comment->user->name) }}" class="rounded-circle me-3 shadow-sm" width="40" height="40">
