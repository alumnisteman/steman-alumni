...
plink -batch -no-antispoof -pw $Password "${Username}@${HostIP}" "rm -rf /home/steman/steman-alumni 2>/dev/null; rm -rf /root/steman-alumni 2>/dev/null"
...
plink -batch -no-antispoof -pw $Password "${Username}@${HostIP}" "docker exec $ContainerName chown -R 82:82 /var/www/storage /var/www/bootstrap/cache; docker exec $ContainerName chmod -R 775 /var/www/storage /var/www/bootstrap/cache; docker restart steman_nginx"
